...
object OpOpenCorridorPortal : Action {
    ...
    internal fun completeDeferredOpen(server: net.minecraft.server.MinecraftServer, request: DeferredPortalOpenRequest) {
        ...
        val sourcePortalPositions = if (request.permanentFrameFlow) {
            request.sourceFrame?.let { PermanentThresholdFrames.aperturePositions(it).toList() }
                ?: listOf(request.sourcePos)
        } else {
            listOf(request.sourcePos)
        }
        val targetPortalPositions = if (request.permanentFrameFlow) {
            request.targetFrame?.let { PermanentThresholdFrames.aperturePositions(it).toList() }
                ?: listOf(request.targetPos)
        } else {
            listOf(request.targetPos)
        }
        if (sourcePortalPositions.size != targetPortalPositions.size) {
            throw MishapPortalNoSpace()
        }

        val portalPairs = sourcePortalPositions.zip(targetPortalPositions)

        val sourcePlacements = sourcePortalPositions.map { pos ->
            preparePortalPlacement(sourceLevel, pos, request.sourceAxis, request.permanentFrameFlow)
        }
        val targetPlacements = targetPortalPositions.map { pos ->
            preparePortalPlacement(targetLevel, pos, request.targetAxis, request.permanentFrameFlow)
        }
        val styledFrameSnapshots = mutableListOf<Pair<net.minecraft.server.level.ServerLevel, PermanentThresholdFrames.RingStateSnapshot>>()

        sourcePlacements.forEach { placement -> applyPortalPlacement(sourceLevel, placement) }
        try {
            targetPlacements.forEach { placement -> applyPortalPlacement(targetLevel, placement) }

            if (request.permanentFrameFlow) {
                request.sourceFrame?.let { frame ->
                    styledFrameSnapshots.add(sourceLevel to PermanentThresholdFrames.applyStyledFrameRing(sourceLevel, frame))
                }
                request.targetFrame?.let { frame ->
                    if (targetLevel != sourceLevel || frame != request.sourceFrame) {
                        styledFrameSnapshots.add(targetLevel to PermanentThresholdFrames.applyStyledFrameRing(targetLevel, frame))
                    }
                }
            }

            for ((sourcePos, targetPos) in portalPairs) {
                val aPortal = sourceLevel.getBlockEntity(sourcePos) as? CorridorPortalBlockEntity
                    ?: throw MishapPortalNoSpace()
                val bPortal = targetLevel.getBlockEntity(targetPos) as? CorridorPortalBlockEntity
                    ?: throw MishapPortalNoSpace()
            ...
            val labelPortal = sourceLevel.getBlockEntity(request.sourcePos) as? CorridorPortalBlockEntity
            labelPortal?.setPortalLabel(request.portalLabel)
            ...
            for ((level, snapshot) in styledFrameSnapshots.asReversed()) {
                PermanentThresholdFrames.restoreStyledFrameRing(level, snapshot)
            }
            sourcePlacements.asReversed().forEach { placement -> rollbackPortalPlacement(sourceLevel, placement) }
            targetPlacements.asReversed().forEach { placement -> rollbackPortalPlacement(targetLevel, placement) }
            throw t
        }
    }
    ...
    private fun preparePortalPlacement(
        ...
        if (state.block == ManifestationBlocks.CORRIDOR_PORTAL_BLOCK) {
            return PortalPlacement(
                ...
                state.getValue(CorridorPortalBlock.THRESHOLD)
            )
        }
        ...
        return PortalPlacement(pos, axis, threshold, false, null, null)
    }
    ...
    private fun applyPortalPlacement(level: net.minecraft.server.level.ServerLevel, placement: PortalPlacement) {
        ...
        if (placement.hasExistingPortal) {
            ...
            val desired = state
                ...
                .setValue(CorridorPortalBlock.THRESHOLD, placement.threshold)
            if (state != desired) {
    ...
    private fun rollbackPortalPlacement(level: net.minecraft.server.level.ServerLevel, placement: PortalPlacement) {
        ...
        if (placement.hasExistingPortal) {
            ...
            val previousThreshold = placement.previousThreshold ?: return
            val desired = currentState
                .setValue(CorridorPortalBlock.AXIS, previousAxis)
                .setValue(CorridorPortalBlock.THRESHOLD, previousThreshold)
            if (currentState != desired) {
                level.setBlock(placement.pos, desired, Block.UPDATE_ALL)
            }
            return
        }
